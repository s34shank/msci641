...
def preprocess_data(df, stemmer=None, stop_words=None):
    def concatenate_text(row):
        post_text = ' '.join([preprocess_text(p, stemmer, stop_words) for p in row['postText'] if isinstance(p, str)])
        target_paragraphs = ' '.join([preprocess_text(p, stemmer, stop_words) for p in row['targetParagraphs'] if isinstance(p, str)])
        return post_text + ' ' + target_paragraphs

    if 'postText' in df.columns and 'targetParagraphs' in df.columns:
        df['combinedText'] = df.apply(concatenate_text, axis=1)
    return df
...
def generate_predictions_csv(model, tokenizer, test_df, spoiler_type_map, output_file='deberta_predictions.csv'):
    test_encodings = tokenize_data(test_df['combinedText'], tokenizer)
    test_dataset = TextDataset(test_encodings, [0] * len(test_df))  
    
    predictions, _ = model.predict(test_df['combinedText'].tolist())
...
def main():
    ...
    train_encodings = tokenize_data(train_df['combinedText'], tokenizer)
    val_encodings = tokenize_data(val_df['combinedText'], tokenizer)
    test_encodings = tokenize_data(test_df['combinedText'], tokenizer)
    ...
    model.save_model('outputs/deberta_model')
